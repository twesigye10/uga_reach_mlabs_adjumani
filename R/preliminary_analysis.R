...
# filter refugee
df_ref_with_composites <- df_data_with_composites %>% 
    filter(status %in% c("refugee"))
ref_svy <- as_survey(.data = df_ref_with_composites, strata = strata)
...
# filter host
df_host_with_composites <- df_data_with_composites %>% 
    filter(status %in% c("host_community"))

host_svy <- as_survey(.data = df_host_with_composites, strata = strata)
