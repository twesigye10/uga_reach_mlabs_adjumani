...
df_for_host_colnames <- df_host_samples %>% 
    ...
    rename(meta_village_name =  village_cell ) %>%
    mutate(meta_division_name = subcounty_division_settlement,
           meta_division_name = str_replace_all(string = district_name, pattern = " ", replacement = "_"),
           village_cell = str_to_lower(village_cell),
           village_cell = str_replace_all(string = village_cell, pattern = " ", replacement = "_")
    ) %>% 
    select(meta_division_name, village_cell) %>% 
    unique()

df_host_samples_required <- df_host_samples %>% 
    rename(village_cell =  village_cell ) %>%
    select(meta_division_name, sample_size) %>% 
    mutate(meta_division_name = str_to_lower(meta_division_name),
           meta_division_name = str_replace_all(string = meta_division_name, pattern = " ", replacement = "_")
    )
